...
export function isOpeningChecklistSkipped(raw) {
  return Boolean(raw && typeof raw === 'object' && raw.skipped === true);
}

export function isOpeningChecklistComplete(raw) {
  return Boolean(
    raw &&
      typeof raw === 'object' &&
      raw.skipped !== true &&
...
export function normalizeOpeningChecklistRecord(raw) {
  ...
  if (raw.skipped === true) {
    const skippedAt =
      typeof raw.skippedAt === 'string' ? raw.skippedAt.trim() : '';
    if (!skippedAt || Number.isNaN(Date.parse(skippedAt))) {
      return { error: 'El checklist saltado no tiene una fecha válida' };
    }
    const by = raw.skippedBy;
    const skippedById =
      by && typeof by === 'object' && typeof by.id === 'string'
        ? by.id.trim()
        : '';
    const skippedByName =
    ...
    if (!skippedById || !skippedByName) {
      return { error: 'El checklist saltado no tiene quién lo omitió' };
    }
    return {
      record: {
        skipped: true,
        skippedAt,
        skippedBy: { id: skippedById, name: skippedByName },
      },
    };
  }
