...
export const TICKET_MENU_ORDER_NOTES = 'Pagado vía ticketera';
...
export function parseEventTicketMenuPicks(raw) {
  ...
  for (const row of raw) {
    const id =
      typeof row === 'string'
        ? String(row || '').trim()
        : String(row?.id || row?.menu_item_id || '').trim();
...
export async function saveEventTicketMenuItems(client, rentalId, menuPicks) {
  const picks = parseEventTicketMenuPicks(menuPicks);
  await client.query(
  ...
  const existing = await client.query(
    `SELECT id, price FROM menu_items WHERE id = ANY($1::text[]) AND archived = 0`,
    [picks.map((pick) => pick.id)],
  );
  const byId = new Map(existing.rows.map((row) => [row.id, row]));
  const ordered = picks.filter((pick) => byId.has(pick.id));
  for (const [index, pick] of ordered.entries()) {
    const menuPrice = Number(byId.get(pick.id).price);
    const price = pick.price == null ? menuPrice : pick.price;
    await client.query(
      `INSERT INTO event_ticket_menu_items (rental_id, menu_item_id, position, price)
       VALUES ($1, $2, $3, $4)`,
      [rentalId, pick.id, index, price],
    );
  }
}
