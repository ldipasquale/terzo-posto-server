...
function buildMultiHtml({
  ...
  const summary = lines
    .map(
      (line) =>
        `${escapeHtml(String(line.quantity))} × ${escapeHtml(line.typeName)}`,
    )
    .join('<br>');
  const qrBlocks = lines
    .map(
      (line, index) => `<tr>
              <td align="center" style="padding:0 8px 8px;color:${CREAM};font-size:14px;line-height:1.5;text-align:center;">
                ${escapeHtml(String(line.quantity))} × ${escapeHtml(line.typeName)}
              </td>
            </tr>
            <tr>
              <td align="center" style="padding:0 8px 20px;text-align:center;">
                <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:0 auto;background:${ORANGE};border-radius:16px;">
                  <tr>
                    <td style="padding:20px;">
                      <img src="cid:ticket-qr-${index}" width="240" height="240" alt="QR de tu entrada" style="display:block;border-radius:12px;background:${CREAM};" />
                    </td>
                  </tr>
                </table>
              </td>
      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:${NAVY};">
            ...
            <tr>
              <td style="padding:0 8px 16px;color:${CREAM};font-size:16px;line-height:1.5;text-align:center;">
                ${greeting ? `${escapeHtml(greeting)}, estas son tus entradas` : 'Estas son tus entradas'}
              </td>
            </tr>
            <tr>
              <td style="padding:0 8px 6px;color:${CREAM};font-size:14px;line-height:1.5;text-align:center;">
                ${summary} · ${formatArs(total)}
              </td>
            </tr>
            <tr>
              <td style="padding:0 8px 6px;color:${CREAM};font-size:14px;line-height:1.5;text-align:center;">
...
export async function sendTicketEmailForRows(rows, venue) {
  if (!rows?.length) return false;
  if (rows.length === 1) return sendTicketEmailForRow(rows[0], venue);
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const png = await QRCode.toBuffer(line.ticketId, {
      type: 'png',
      width: 480,
      margin: 2,
      errorCorrectionLevel: 'M',
    ...
    attachments.push({
      filename: `entrada-${line.ticketId}.png`,
      content: png.toString('base64'),
  ...
  const { error } = await postResendEmail(apiKey, {
    from,
    to: [String(to).trim()],
    subject: `Entradas · ${first.activity_name} en El Terzo Posto`,
    html: buildMultiHtml({
      buyerName: first.buyer_name,
      eventName: first.activity_name,
      when: formatEventWhen(first.date, first.start_time),
      lines,
      total: Math.max(0, lines.reduce((sum, line) => sum + purchaseLineTotal(line), 0)),
      address: venueAddressLine(place),
      directionsUrl: mapsDirectionsUrl(place),
    }),
    attachments,
